...
public class DoubleDCT_1D
{
	...
	public DoubleDCT_1D(int n)
	{
		...
		if (ConcurrencyUtils.isPowerOf2(n))
		{
			isPowerOfTwo = true;
			ip = new int[(int) Math
					.ceil(2 + (1 << (int) (Math.log(n / 2 + 0.5) / Math.log(2)) / 2))];
			w = new double[n * 5 / 4];
			nw = ip[0];
			if (n > nw << 2)
			{
	...
	private void dctsub(int n, double[] a, int offa, int nc, double[] c,
			int startc)
	{
		int k, kk, ks, m;
		double wkr, wki, xr;
	...
	private double[] makect(int n)
	{
		int twon = 2 * n;
		int idx;
		double delta = DoubleDCT_1D.PI / twon;
		double deltaj;
