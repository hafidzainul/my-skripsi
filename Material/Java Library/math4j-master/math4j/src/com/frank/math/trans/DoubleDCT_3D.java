				...
				public void run()
				{
					...
					if (isgn == -1)
						for (int s = n0; s < slices; s += nthreads)
						{
							idx0 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dctColumns.forward(a, idx0 + r * rowStride,
										scale);
							if (columns > 2)
								for (int c = 0; c < columns; c += 4)
								{
						...
						for (int s = n0; s < slices; s += nthreads)
						{
							idx0 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dctColumns.inverse(a, idx0 + r * rowStride,
										scale);
							if (columns > 2)
								for (int c = 0; c < columns; c += 4)
								{
						...
						if (columns > 2)
							for (int r = n0; r < rows; r += nthreads)
							{
								...
								for (int c = 0; c < columns; c += 4)
								{
									...
									dctSlices.forward(t, startt + 2 * slices,
											scale);
									dctSlices.forward(t, startt + 3 * slices,
											scale);
									for (int s = 0; s < slices; s++)
									{
										idx1 = s * sliceStride + idx0 + c;
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							...
							for (int r = 0; r < rows; r++)
								dctColumns.forward(a, idx1 + r * rowStride,
										scale);
						}
					}
				});
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							int idx1 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dctColumns.inverse(a, idx1 + r * rowStride,
										scale);
						}
					}
				});
