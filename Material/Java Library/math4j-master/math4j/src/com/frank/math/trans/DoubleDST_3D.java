...
public class DoubleDST_3D
{
	...
	private void ddxt3da_subth(final int isgn, final double[] a,
			final boolean scale)
	{
				...
				public void run()
				{
					...
					if (isgn == -1)
						for (int s = n0; s < slices; s += nthreads)
						{
							idx0 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dstColumns.forward(a, idx0 + r * rowStride,
										scale);
							if (columns > 2)
								for (int c = 0; c < columns; c += 4)
								{
						...
						for (int s = n0; s < slices; s += nthreads)
						{
							idx0 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dstColumns.inverse(a, idx0 + r * rowStride,
										scale);
							if (columns > 2)
								for (int c = 0; c < columns; c += 4)
								{
	...
	private void ddxt3db_subth(final int isgn, final double[] a,
			final boolean scale)
	{
					...
					if (isgn == -1)
					{
						if (columns > 2)
							for (int r = n0; r < rows; r += nthreads)
							{
								...
								for (int c = 0; c < columns; c += 4)
								{
									...
									dstSlices
											.forward(t, startt + slices, scale);
									dstSlices.forward(t, startt + 2 * slices,
											scale);
									dstSlices.forward(t, startt + 3 * slices,
											scale);
									for (int s = 0; s < slices; s++)
									{
										idx1 = s * sliceStride + idx0 + c;
						...
						for (int r = n0; r < rows; r += nthreads)
						{
							...
							for (int c = 0; c < columns; c += 4)
							{
								...
								dstSlices
										.inverse(t, startt + 2 * slices, scale);
								dstSlices
										.inverse(t, startt + 3 * slices, scale);
								for (int s = 0; s < slices; s++)
								{
									idx1 = s * sliceStride + idx0 + c;
									idx2 = startt + slices + s;
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							...
							for (int r = 0; r < rows; r++)
								dstColumns.forward(a, idx1 + r * rowStride,
										scale);
						}
					}
				});
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							int idx1 = s * sliceStride;
							for (int r = 0; r < rows; r++)
								dstColumns.inverse(a, idx1 + r * rowStride,
										scale);
						}
					}
				});
