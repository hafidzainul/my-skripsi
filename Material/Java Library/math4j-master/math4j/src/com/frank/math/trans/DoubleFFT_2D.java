...
public class DoubleFFT_2D
{
	...
	private void cdft2d_subth(final int isgn, final double[] a,
			final boolean scale)
	{
	...
	public void complexForward(final double[] a)
	{
		...
		else
		{
			...
			if (nthreads > 1 && useThreads && rows >= nthreads
					&& columns >= nthreads)
			{
				...
				for (int l = 0; l < nthreads; l++)
				{
					...
					final int lastColumn = l == nthreads - 1 ? columns
							: firstColumn + p;
					futures[l] = ConcurrencyUtils.submit(new Runnable()
					{
						@Override
	...
	public void complexForward(final double[][] a)
	{
		...
		else if (nthreads > 1 && useThreads && rows >= nthreads
				&& columns >= nthreads)
		{
			Future<?>[] futures = new Future[nthreads];
			int p = rows / nthreads;
				...
				for (int l = 0; l < nthreads; l++)
				{
					...
					futures[l] = ConcurrencyUtils.submit(new Runnable()
					{
						...
						public void run()
						{
							for (int r = firstRow; r < lastRow; r++)
								fftColumns
										.complexInverse(a, r * rowspan, scale);
						}
					});
				}
				ConcurrencyUtils.waitForCompletion(futures);
	...
	private void mixedRadixRealForwardFull(final double[] a)
	{
		...
		if (nthreads > 1 && useThreads && rows >= nthreads && n2d2 >= nthreads)
		{
			...
			for (int l = 0; l < nthreads; l++)
			{
				...
				final int lastColumn = l == nthreads - 1 ? n2d2 - 1
						: firstColumn + p;
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					@Override
	...
	private void xdft2d0_subth1(final int icr, final int isgn,
				...
				public void run()
				{
					if (icr == 0)
					{
						if (isgn == -1)
							...
							for (int r = n0; r < rows; r += nthreads)
								fftColumns
										.complexInverse(a, r * columns, scale);
					}
					else if (isgn == 1)
						for (int r = n0; r < rows; r += nthreads)
							fftColumns.realForward(a, r * columns);
	...
	private void xdft2d0_subth2(final int icr, final int isgn,
			final double[] a, final boolean scale)
	{
		final int nthreads = ConcurrencyUtils.getNumberOfThreads() > rows ? rows
				: ConcurrencyUtils.getNumberOfThreads();
