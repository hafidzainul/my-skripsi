...
public class FFT2D {
     ...
     *
     * @param x the specified complex matrix, <code>x</code> must arranged as
    ...
    public /*static*/ Complex[][] fft(Complex[][] x) {
        ...
        for (int j = 0; j < nrows; j++) {
            for (int i = 0; i < ncols; i++) {
                data[j * ncols * 2 + 2 * i] = x[j][i].real;
                data[j * ncols * 2 + 2 * i + 1] = x[j][i].imaginary;
            }
        }
     ...
     *
     * @param y the specified complex matrix, <code>y</code> must arranged as
    ...
    public /*static*/ Complex[][] ifft(Complex[][] y, boolean scale) {
        ...
        for (int j = 0; j < nrows; j++) {
            for (int i = 0; i < ncols; i++) {
                data[j * ncols * 2 + 2 * i] = y[j][i].real;
                data[j * ncols * 2 + 2 * i + 1] = y[j][i].imaginary;
            }
        }
