 ...
 *
 * ***** END LICENSE BLOCK ***** */
package com.frank.math.trans;

import java.util.concurrent.Future;

import com.frank.math.struct.ConcurrencyUtils;
...
public class DoubleFFT_1D
{
	...
	public DoubleFFT_1D(int n)
	{
		...
		{
			if (DoubleFFT_1D.getReminder(n, DoubleFFT_1D.factors) >= 211)
			{
				plan = Plans.BLUESTEIN;
				nBluestein = ConcurrencyUtils.nextPow2(n * 2 - 1);
				bk1 = new double[2 * nBluestein];
				bk2 = new double[2 * nBluestein];
				ip = new int[2 + (int) Math.ceil(2 + (1 << (int) (Math
						.log(nBluestein + 0.5) / Math.log(2)) / 2))];
				w = new double[nBluestein];
		...
		else
		{
			...
			ip = new int[2 + (int) Math
					.ceil(2 + (1 << (int) (Math.log(n + 0.5) / Math.log(2)) / 2))];
			w = new double[n];
			int twon = 2 * n;
			nw = ip[0];
			if (twon > nw << 2)
	...
	private void bluestein_complex(final double[] a, final int offa,
			final int isign)
	{
				...
				&& n > ConcurrencyUtils.getThreadsBeginN_1D_FFT_2Threads())
		{
			nthreads = 2;
			if (nthreads >= 4
					&& n > ConcurrencyUtils.getThreadsBeginN_1D_FFT_4Threads())
				nthreads = 4;
					...
					public void run()
					{
						if (isign > 0)
							for (int i = firstIdx; i < lastIdx; i++)
							{
								...
								int idx3 = offa + idx1;
								int idx4 = offa + idx2;
								ak[idx1] = a[idx3] * bk1[idx1] - a[idx4]
										* bk1[idx2];
								ak[idx2] = a[idx3] * bk1[idx2] + a[idx4]
										* bk1[idx1];
							}
						else
							for (int i = firstIdx; i < lastIdx; i++)
			...
			{
				final int firstIdx = i * k;
				final int lastIdx = i == nthreads - 1 ? nBluestein : firstIdx
						+ k;
				futures[i] = ConcurrencyUtils.submit(new Runnable()
				{
					@Override
					public void run()
					{
						if (isign > 0)
							for (int i = firstIdx; i < lastIdx; i++)
							{
								int idx1 = 2 * i;
								int idx2 = idx1 + 1;
								double im = -ak[idx1] * bk2[idx2] + ak[idx2]
										* bk2[idx1];
								ak[idx1] = ak[idx1] * bk2[idx1] + ak[idx2]
										* bk2[idx2];
								ak[idx2] = im;
							}
						else
					...
					public void run()
					{
						if (isign > 0)
							for (int i = firstIdx; i < lastIdx; i++)
							{
								...
								int idx3 = offa + idx1;
								int idx4 = offa + idx2;
								a[idx3] = bk1[idx1] * ak[idx1] - bk1[idx2]
										* ak[idx2];
								a[idx4] = bk1[idx2] * ak[idx1] + bk1[idx1]
										* ak[idx2];
							}
						else
							for (int i = firstIdx; i < lastIdx; i++)
	...
	private void bluestein_real_full(final double[] a, final int offa,
			final int isign)
	{
		final double[] ak = new double[2 * nBluestein];
		int nthreads = ConcurrencyUtils.getNumberOfThreads();
					...
					public void run()
					{
						if (isign > 0)
							for (int i = firstIdx; i < lastIdx; i++)
							{
								int idx1 = 2 * i;
								int idx2 = idx1 + 1;
								a[offa + idx1] = bk1[idx1] * ak[idx1]
										- bk1[idx2] * ak[idx2];
								a[offa + idx2] = bk1[idx2] * ak[idx1]
										+ bk1[idx1] * ak[idx2];
							}
						else
							for (int i = firstIdx; i < lastIdx; i++)
		...
		else
		{
			...
			if (isign > 0)
				for (int i = 0; i < n; i++)
				{
					int idx1 = 2 * i;
					int idx2 = idx1 + 1;
					a[offa + idx1] = bk1[idx1] * ak[idx1] - bk1[idx2]
							* ak[idx2];
					a[offa + idx2] = bk1[idx2] * ak[idx1] + bk1[idx1]
							* ak[idx2];
				}
			else
				for (int i = 0; i < n; i++)
				...
				futures[i] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int i = firstIdx; i < lastIdx; i++)
						{
							int idx1 = 2 * i;
							int idx2 = idx1 + 1;
							a[offa + i] = bk1[idx1] * ak[idx1] - bk1[idx2]
									* ak[idx2];
						}
					}
				});
	...
	void cfftf(double a[], int offa, int isign)
	{
		...
		for (int k1 = 2; k1 <= nf + 1; k1++)
		{
			...
			switch (ip)
			{
				...
				default:
					if (na == 0)
						passfg(nac, idot, ip, l1, idl1, a, offa, ch, 0, iw,
								isign);
					else
						passfg(nac, idot, ip, l1, idl1, ch, 0, a, offa, iw,
								isign);
					if (nac[0] != 0)
						na = 1 - na;
					break;
	...
	private void cftbsub(int n, double[] a, int offa, int[] ip, int nw,
			double[] w)
	{
		if (n > 8)
		{
	...
	private void cftfsub(int n, double[] a, int offa, int[] ip, int nw,
			double[] w)
	{
		if (n > 8)
		{
	...
	private void cftleaf(int n, int isplt, double[] a, int offa, int nw,
			double[] w)
	{
		if (n == 512)
		{
	...
	private void cftrec4_th(final int n, final double[] a, final int offa,
			final int nw, final double[] w)
	{
		int i;
	...
	private int cfttree(int n, int j, int k, double[] a, int offa, int nw,
			double[] w)
	{
		int i, isplt, m;
		int idx1 = offa - n;
	...
	void passf2(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
	...
	/*----------------------------------------------------------------------
	   ...
	   isign is +1 for backward and -1 for forward transforms
	  ----------------------------------------------------------------------*/
	void passf3(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset, final int isign)
	{
	...
	/*----------------------------------------------------------------------
	   ...
	   isign is +1 for backward and -1 for forward transforms
	  ----------------------------------------------------------------------*/
	void passf4(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset, final int isign)
	{
	...
	/*----------------------------------------------------------------------
	   ...
	   isign is +1 for backward and -1 for forward transforms
	  ----------------------------------------------------------------------*/
	void passf5(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset, final int isign)
	...
	/*----------------------------------------------------------------------
	   ...
	   isign is +1 for backward and -1 for forward transforms
	  ----------------------------------------------------------------------*/
	void passfg(final int nac[], final int ido, final int ip, final int l1,
			final int idl1, final double in[], final int in_off,
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radb2(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
		int i, ic;
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radb3(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
		final double taur = -0.5;
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radb4(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radb5(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
		final double tr11 = 0.309016994374947451262869435595348477;
	...
	/*---------------------------------------------------------
	  ...
	  --------------------------------------------------------*/
	void radbg(final int ido, final int ip, final int l1, final int idl1,
			final double in[], final int in_off, final double out[],
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radf2(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radf3(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
		final double taur = -0.5;
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radf4(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
	...
	/*-------------------------------------------------
	  ...
	  -------------------------------------------------*/
	void radf5(final int ido, final int l1, final double in[],
			final int in_off, final double out[], final int out_off,
			final int offset)
	{
		final double tr11 = 0.309016994374947451262869435595348477;
	...
	/*---------------------------------------------------------
	  ...
	  --------------------------------------------------------*/
	void radfg(final int ido, final int ip, final int l1, final int idl1,
			final double in[], final int in_off, final double out[],
			final int out_off, final int offset)
	{
		...
		switch (plan)
		{
			case SPLIT_RADIX:
								...
								.getThreadsBeginN_1D_FFT_2Threads())
				{
					...
					{
						final int firstIdx = i * k;
						final int lastIdx = i == nthreads - 1 ? n / 2
								: firstIdx + k;
						futures[i] = ConcurrencyUtils.submit(new Runnable()
						{
							@Override
	...
	private void rftbsub(int n, double[] a, int offa, int nc, double[] c,
			int startc)
	{
		int k, kk, ks, m;
	...
	private void rftfsub(int n, double[] a, int offa, int nc, double[] c,
			int startc)
	{
		int k, kk, ks, m;
		double wkr, wki, xr, xi, yr, yi;
			...
			boolean complex)
	{
		...
		int nthreads = ConcurrencyUtils.getNumberOfThreads();
		if (nthreads > 1
				&& n2 >= ConcurrencyUtils.getThreadsBeginN_1D_FFT_2Threads())
		{
			...
			for (int i = 0; i < nthreads; i++)
			{
				...
				final int lastIdx = i == nthreads - 1 ? offa + n2 : firstIdx
						+ k;
				futures[i] = ConcurrencyUtils.submit(new Runnable()
				{
					@Override
