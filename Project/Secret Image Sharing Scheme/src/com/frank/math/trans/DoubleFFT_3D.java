...
public class DoubleFFT_3D
{
	...
	public DoubleFFT_3D(int slices, int rows, int columns)
	{
		...
		if (ConcurrencyUtils.isPowerOf2(slices)
				&& ConcurrencyUtils.isPowerOf2(rows)
	...
	private void cdft3db_subth(final int isgn, final double[] a,
			final boolean scale)
	{
		int nt, i;
		final int nthreads = Math.min(ConcurrencyUtils.getNumberOfThreads(),
				rows);
		nt = 8 * slices;
		if (columns == 4)
			nt >>= 1;
					...
					if (isgn == -1)
					{
						if (columns > 4)
							for (int r = n0; r < rows; r += nthreads)
							{
								...
								for (int c = 0; c < columns; c += 8)
								{
									...
									fftSlices.complexForward(t, startt + 2
											* slices);
									fftSlices.complexForward(t, startt + 4
											* slices);
									fftSlices.complexForward(t, startt + 6
											* slices);
									for (int s = 0; s < slices; s++)
									{
										idx1 = s * sliceStride + idx0 + c;
							...
							for (int r = n0; r < rows; r += nthreads)
							{
								...
								fftSlices.complexForward(t, startt);
								fftSlices
										.complexForward(t, startt + 2 * slices);
								for (int s = 0; s < slices; s++)
								{
									idx1 = s * sliceStride + idx0;
									idx2 = startt + 2 * s;
						...
						for (int r = n0; r < rows; r += nthreads)
						{
							...
							for (int c = 0; c < columns; c += 8)
							{
								...
								fftSlices.complexInverse(t,
										startt + 2 * slices, scale);
								fftSlices.complexInverse(t,
										startt + 4 * slices, scale);
								fftSlices.complexInverse(t,
										startt + 6 * slices, scale);
								for (int s = 0; s < slices; s++)
								{
									idx1 = s * sliceStride + idx0 + c;
									idx2 = startt + 2 * s;
						...
						for (int r = n0; r < rows; r += nthreads)
						{
							...
							fftSlices.complexInverse(t, startt, scale);
							fftSlices.complexInverse(t, startt + 2 * slices,
									scale);
							for (int s = 0; s < slices; s++)
							{
								idx1 = s * sliceStride + idx0;
	...
	public void complexForward(final double[] a)
	{
		...
		else
		{
			...
			if (nthreads > 1 && useThreads && slices >= nthreads
					&& rows >= nthreads && columns >= nthreads)
			{
				Future<?>[] futures = new Future[nthreads];
				...
				for (int l = 0; l < nthreads; l++)
				{
					...
					final int lastSlice = l == nthreads - 1 ? slices
							: firstSlice + p;
					futures[l] = ConcurrencyUtils.submit(new Runnable()
					{
						...
						public void run()
						{
							for (int s = firstSlice; s < lastSlice; s++)
							{
								...
								for (int r = 0; r < rows; r++)
									fftColumns.complexForward(a, idx1 + r
											* rowStride);
							}
						}
					});
	...
	public void complexForward(final double[][][] a)
	{
		...
		else if (nthreads > 1 && useThreads && slices >= nthreads
				&& rows >= nthreads && columns >= nthreads)
		{
			Future<?>[] futures = new Future[nthreads];
			int p = slices / nthreads;
			...
			{
				final int firstSlice = l * p;
				final int lastSlice = l == nthreads - 1 ? slices : firstSlice
						+ p;
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					@Override
		...
		else
		{
					...
					futures[l] = ConcurrencyUtils.submit(new Runnable()
					{
						...
						public void run()
						{
							for (int s = firstSlice; s < lastSlice; s++)
							{
								int idx1 = s * sliceStride;
								for (int r = 0; r < rows; r++)
									fftColumns.complexInverse(a, idx1 + r
											* rowStride, scale);
							}
						}
					});
			...
			else
			{
				for (int s = 0; s < slices; s++)
				{
					int idx1 = s * sliceStride;
					for (int r = 0; r < rows; r++)
						fftColumns.complexInverse(a, idx1 + r * rowStride,
								scale);
				}
				double[] temp = new double[2 * rows];
				for (int s = 0; s < slices; s++)
	...
	private void mixedRadixRealForwardFull(final double[] a)
	{
		...
		if (nthreads > 1 && useThreads && n1d2 >= nthreads
				&& columns >= nthreads && ldimn2 >= nthreads)
		{
			Future<?>[] futures = new Future[nthreads];
			int p = n1d2 / nthreads;
			for (int l = 0; l < nthreads; l++)
			{
				...
				final int lastSlice = l == nthreads - 1 ? n1d2 + 1 : firstSlice
						- p;
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					@Override
					public void run()
					{
						...
						for (int s = firstSlice; s >= lastSlice; s--)
						{
							...
							{
								System.arraycopy(a, idx1 + r * rowStride, temp,
										0, columns);
								fftColumns.realForwardFull(temp);
								System.arraycopy(temp, 0, a, idx2 + r
										* twoRowStride, twon3);
							}
						}
					}
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							...
							for (int r = 0; r < rows; r++)
							{
								System.arraycopy(a, idx1 + r * rowStride,
										temp2[s][r], 0, columns);
				...
				futures[l] = ConcurrencyUtils.submit(new Runnable()
				{
					...
					public void run()
					{
						for (int s = firstSlice; s < lastSlice; s++)
						{
							...
							for (int r = 0; r < rows; r++)
								System.arraycopy(temp2[s][r], 0, a, idx1 + r
										* twoRowStride, twon3);
						}
					}
				});
	...
	private void mixedRadixRealInverseFull(final double[][][] a,
			final boolean scale)
	{
		double[] temp = new double[2 * rows];
		int ldimn2 = rows / 2 + 1;
	...
	private void xdft3da_sub1(int icr, int isgn, double[] a, boolean scale)
	{
		...
		if (isgn == -1)
			...
			for (int s = 0; s < slices; s++)
			{
				...
				if (icr == 0)
					for (int r = 0; r < rows; r++)
						fftColumns.complexInverse(a, idx0 + r * rowStride,
								scale);
				if (columns > 4)
					for (int c = 0; c < columns; c += 8)
					{
	...
	private void xdft3da_subth1(final int icr, final int isgn,
			final double[] a, final boolean scale)
	{
		int nt, i;
		final int nthreads = Math.min(ConcurrencyUtils.getNumberOfThreads(),
				slices);
				...
				public void run()
				{
					...
					if (isgn == -1)
						for (int s = n0; s < slices; s += nthreads)
						{
							...
							if (icr == 0)
								for (int r = 0; r < rows; r++)
									fftColumns.complexForward(a, idx0 + r
											* rowStride);
							else
								for (int r = 0; r < rows; r++)
									fftColumns.realForward(a, idx0 + r
											* rowStride);
							if (columns > 4)
								for (int c = 0; c < columns; c += 8)
								{
						...
						for (int s = n0; s < slices; s += nthreads)
						{
							...
							if (icr == 0)
								for (int r = 0; r < rows; r++)
									fftColumns.complexInverse(a, idx0 + r
											* rowStride, scale);
							if (columns > 4)
								for (int c = 0; c < columns; c += 8)
								{
							...
							else if (columns == 4)
							{
								...
								fftRows.complexInverse(t, startt, scale);
								fftRows.complexInverse(t, startt + 2 * rows,
										scale);
								for (int r = 0; r < rows; r++)
								{
									idx1 = idx0 + r * rowStride;
	...
	private void xdft3da_subth2(final int icr, final int isgn,
			final double[] a, final boolean scale)
	{
		int nt, i;
		final int nthreads = Math.min(ConcurrencyUtils.getNumberOfThreads(),
					...
					if (isgn == -1)
						...
						for (int s = n0; s < slices; s += nthreads)
						{
							...
							if (icr == 0)
								...
								for (int r = 0; r < rows; r++)
									fftColumns.realInverse2(a, idx0 + r
											* rowStride, scale);
							if (columns > 4)
								for (int c = 0; c < columns; c += 8)
								{
